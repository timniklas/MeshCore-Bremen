...
void MyMesh::handleCmdFrame(size_t len) {
  if (cmd_frame[0] == CMD_DEVICE_QEURY && len >= 2) { // sent when app establishes connection
    ...
    if (txt_type != TXT_TYPE_PLAIN) {
      ...
      // === Neu: "name: nachricht" unterstützen ===
      const char *sender = _prefs.node_name;   // Standard: eigener Node-Name
      const char *final_text = text;           // Standard: kompletter Text
      int final_len = tlen;

      char tmp_name[33];                       // Annahme: max. 32 Zeichen Name (+ '\0')
      const char *colon = (const char *)memchr(text, ':', tlen);

      if (colon) {
        int prefix_len = (int)(colon - text);  // Länge vor dem ':'
        if (prefix_len > 0) {                  // Leerer Prefix -> ignoriere (fallback auf _prefs.node_name)
          if (prefix_len > 32) prefix_len = 32; // begrenzen um Überlauf zu vermeiden
          memcpy(tmp_name, text, prefix_len);
          tmp_name[prefix_len] = 0;            // null-terminieren
          sender = tmp_name;

          // Alles nach dem ':' ist der eigentliche Nachrichtentext
          final_text = colon + 1;
          final_len = tlen - (prefix_len + 1);

          // Optional: führendes Leerzeichen nach dem ':' entfernen
          // while (final_len > 0 && *final_text == ' ') { final_text++; final_len--; }
        }
      }
      // === Ende Neu ===

      if (success && sendGroupMessage(msg_timestamp, channel.channel, sender, final_text, final_len)) {
        writeOKFrame();
      } else {
