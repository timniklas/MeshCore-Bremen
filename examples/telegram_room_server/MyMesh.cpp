...
#ifdef ESP32
  if (!g_isOnline) {
    MESH_DEBUG_PRINTLN("onAnonDataRecv: rejecting anon req while offline");
    return; // refuse to process login when offline -> avoids split-brain
  }
#endif
...
#ifdef ESP32
  if (!g_isOnline) {
    MESH_DEBUG_PRINTLN("onPeerDataRecv: ignoring peer data while offline");
    return; // ignore posts/requests when offline
  }
#endif
...
#ifdef ESP32
  if (!g_isOnline) {
    // When offline, skip room-server behavior to prevent split-brain.
    // Still update uptime counters.
    uint32_t now = millis();
